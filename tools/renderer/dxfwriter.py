...
class DxfCanvas(Canvas):
    ...
    def dxfattribs(self):
        v = {'color': ColorMap[self.styles[-1].lineColor], 'lineweight':25}
    ...
    def path (self,points,closed = False):
        ...
        if closed:
            pts = pts+ [pts[0]]
        pts = [[p[0]+self.offset[0],p[1]-self.offset[1]] for p in pts]
        self.dxf.modelspace().add_lwpolyline(pts, dxfattribs=self.dxfattribs())
        pass
